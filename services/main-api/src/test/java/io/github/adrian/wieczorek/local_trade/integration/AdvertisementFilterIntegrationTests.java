...
public class AdvertisementFilterIntegrationTests extends AbstractIntegrationTest {
    ...
    public void filterByCategoryIdAndPageAdvertisements_thenReturnPageOfAdvertisements() throws Exception {
        ...
        List<AdvertisementEntity> advertisementEntities = advertisementRepository.saveAll(
                IntStream.range(0, 10)
                        .mapToObj(i -> AdUtils.createAdvertisementRoleUserForIntegrationTests(categoryEntity, user))
                        .toList()
        );
        ...
        mockMvc.perform(
                        get("/advertisements/search")
                                .with(csrf())
                                .param("categoryEntityId", String.valueOf(categoryId))
                                .param("page", "0")
                                .param("size", "10")
                )
    ...
    public void filterByCategoryIdAndFilterForTitlePageAdvertisements_thenReturnPage() throws Exception {
        ...
        mockMvc.perform(
                        get("/advertisements/search")
                                .with(csrf())
                                .param("categoryEntityId", String.valueOf(advertisementEntities.get(0).getCategoryEntity().getId()))
