...
class S3FinderUnitTests {
    ...
    void listFiles_whenImagesExist_returnsDtosWithPresignedUrls() {
        when(advertisementService.getCurrentAdvertisement(advertisementId)).thenReturn(advertisementEntity);

        when(imageRepository.findAllByAdvertisementEntity(advertisementEntity)).thenReturn(List.of(imageEntity));
    ...
    void listFiles_whenNoImages_returnsEmptyList() {
        when(advertisementService.getCurrentAdvertisement(advertisementId)).thenReturn(advertisementEntity);
        when(imageRepository.findAllByAdvertisementEntity(advertisementEntity)).thenReturn(Collections.emptyList());
    ...
    void listFiles_whenAdNotFound_throwsException() {
        when(advertisementService.getCurrentAdvertisement(advertisementId)).thenThrow(new EntityNotFoundException("Ad not found"));

        Assertions.assertThrows(EntityNotFoundException.class, () -> s3Finder.listFiles(advertisementId));
